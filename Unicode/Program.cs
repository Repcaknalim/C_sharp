using System.Text;
using System.IO;

namespace Unicode
{
    class Program
    {
        static void Main()
        {
            int intValue = 4869414;
            string stringValue = "Hello";
            byte[] byteArray = { 47, 129, 0, 116 };
            float floatValue = 491.695F;
            char charValue = 'E';

            using (var output = File.Create("binData.txt"))
            using (var writer = new BinaryWriter(output))
            {
                writer.Write(intValue);
                writer.Write(stringValue);
                writer.Write(byteArray);
                writer.Write(floatValue);
                writer.Write(charValue);
            }

            byte[] dataWritten = File.ReadAllBytes("binData.txt");
            foreach (byte b in dataWritten)
                Console.Write("{0:x2} ", b);
            Console.WriteLine("Byte number: - {0}", dataWritten.Length);

            using (var input = File.OpenRead("binData.txt"))
            using (var reader = new BinaryReader(input))
            {
                int intRead = reader.ReadInt32();
                string stringRead = reader.ReadString();
                byte[] byteArrayRead = reader.ReadBytes(4);
                float floatRead = reader.ReadSingle();
                char charRead = reader.ReadChar();

                Console.Write("Int: {0}\nString: {1}\nBytes: ", intRead, stringRead);
                foreach (byte b in byteArrayRead)
                    Console.Write("{0} ", b);
                Console.Write("\nFloat: {0}\nChar: {1}", floatRead, charRead);
            }
        }
    }
}